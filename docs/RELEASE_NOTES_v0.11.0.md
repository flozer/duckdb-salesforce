# duckdb-salesforce v0.11.0

Own-repo release on top of `v0.10.1` (`6deab9d`). Scope: the v1.6 Report Bridge
metadata + explainability track (Phases 1–4) for `salesforce_report_soql()`. No
new SQL functions; the change is richer, describe-validated resolution plus
diagnostic columns. The approved `duckdb/community-extensions` baseline remains
`v0.9.2` — community is **not** updated.
