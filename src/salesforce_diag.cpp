...
string BuildGuidance(const ScanCost &c) {
	...
	if (c.residual_filters > 0) {
		hints.push_back(StringUtil::Format("DuckDB is filtering after a full remote scan: %lld filter(s) applied "
	...
	if (c.count_pushdown) {
		hints.push_back(StringUtil::Format("count pushdown: emitted %lld rows from SELECT COUNT(), no records fetched",
...
unique_ptr<FunctionData> QueryCostBind(ClientContext &, TableFunctionBindInput &, vector<LogicalType> &return_types,
                                       vector<string> &names) {
	names = {
	    "object",       "soql",           "transport",        "est_rows",     "transport_reason", "projected_fields",
	    "total_fields", "pushed_filters", "residual_filters", "where_pushed", "pages_fetched",    "rows_emitted",
	    "bulk",         "count_pushdown", "bulk_chunks",      "query_mode",   "quota_remaining",  "quota_allowed",
	    "guidance",     "bulk_polls"};
	return_types = {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::BIGINT,
	                ...
	                LogicalType::BOOLEAN, LogicalType::BOOLEAN, LogicalType::BIGINT,  LogicalType::VARCHAR,
	                LogicalType::BIGINT,  LogicalType::BOOLEAN, LogicalType::VARCHAR, LogicalType::BIGINT};
	return nullptr;
}
...
void DiagRecordScan(const string &object, const string &soql, const string &transport, int64_t est_rows,
                    const string &transport_reason, int64_t projected_fields, int64_t total_fields,
                    int64_t pushed_filters, int64_t residual_filters, const string &where_pushed, bool bulk,
                    int64_t pages_init, bool count_pushdown, const string &query_mode) {
	std::lock_guard<std::mutex> g(g_lock);
	g_cost = ScanCost {};
...
TableFunction GetSalesforceQueryCostFunction() {
	return TableFunction("salesforce_query_cost", {}, QueryCostFunction, QueryCostBind, QueryCostInit);
}
