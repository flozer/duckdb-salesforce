...
static void AppendRelationshipStruct(Vector &vec, idx_t row, const SalesforceField &field, const string &record_json) {
...
void AppendTypedCell(Vector &vec, idx_t row, const SalesforceField &field, const string &raw) {
	...
	if (type.id() == LogicalTypeId::BLOB) {
		...
		try {
			...
			FlatVector::GetData<string_t>(vec)[row] = StringVector::AddStringOrBlob(vec, bytes.data(), bytes.size());
			return;
			...
			if (IsBlobUrlReference(raw)) {
				throw InvalidInputException("salesforce: Salesforce returned a URL reference for blob/base64 "
			...
			// the content).
			throw InvalidInputException("salesforce: field '%s' (Salesforce type '%s') could not be decoded "
			                            "as BLOB (length=%llu).",
	...
	try {
		...
		throw InvalidInputException("salesforce: field '%s' (Salesforce type '%s') could not be decoded as %s.",
...
struct DecodeBindData : public FunctionData {
	...
	bool Equals(const FunctionData &other_p) const override {
		...
		return records.size() == other.records.size() && fields.size() == other.fields.size();
	}
};
...
static unique_ptr<FunctionData> DecodeBind(ClientContext &, TableFunctionBindInput &input,
	...
	if (input.inputs.size() < 2) {
		throw BinderException("salesforce_decode(fields_json, records_json) requires two arguments");
	}
...
static unique_ptr<GlobalTableFunctionState> DecodeInitGlobal(ClientContext &, TableFunctionInitInput &) {
...
TableFunction GetSalesforceDecodeFunction() {
	return TableFunction("salesforce_decode", {LogicalType::VARCHAR, LogicalType::VARCHAR}, DecodeFunction, DecodeBind,
	                     DecodeInitGlobal);
}
