...
LogicalType MapSalesforceType(const string &sf_type, int64_t precision, int64_t scale, bool *unknown) {
	...
	if (t == "string" || t == "textarea" || t == "id" || t == "reference" || t == "picklist" || t == "multipicklist" ||
	    t == "phone" || t == "email" || t == "url" || t == "encryptedstring" || t == "combobox" || t == "anytype" ||
	...
	if (t == "currency" || t == "percent") {
		if (precision > 0 && precision <= 38) {
			...
			return LogicalType::DECIMAL(static_cast<uint8_t>(precision), static_cast<uint8_t>(s));
		}
		return LogicalType::DOUBLE;
	}
	if (t == "date") {
...
LogicalType MapToolingDataType(const string &data_type, bool *ok) {
	...
	if (t == "text" || t == "text area" || t == "long text area" || t == "rich text area" || t == "email" ||
	    t == "phone" || t == "url" || t == "picklist" || t == "multipicklist" || t == "combobox" || t == "lookup" ||
	    t == "master-detail" || t == "hierarchy" || t == "external lookup" || t == "indirect lookup" || t == "id" ||
	    t == "reference" || t == "auto number" || t == "encrypted text" || t == "data category group reference") {
		return LogicalType::VARCHAR;
	}
	if (t == "checkbox") {
		return LogicalType::BOOLEAN;
