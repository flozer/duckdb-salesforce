...
static unique_ptr<FunctionData> DescribeBind(ClientContext &context, TableFunctionBindInput &input,
                                             vector<LogicalType> &return_types, vector<string> &names) {
	if (input.inputs.empty()) {
		throw BinderException("salesforce_describe(object) requires the sObject name as the first argument");
	}
	...
	cfg.api_version = api_version.empty() ? SalesforceConfig::kDefaultApiVersion : api_version;

	auto require = [&](const string &v, const char *key) {
		if (v.empty()) {
			throw BinderException("salesforce_describe: missing required named parameter '%s'", key);
		}
	};
...
static unique_ptr<GlobalTableFunctionState> DescribeInitGlobal(ClientContext &, TableFunctionInitInput &) {
...
static void DescribeFunction(ClientContext &, TableFunctionInput &data, DataChunk &output) {
	...
	while (row < STANDARD_VECTOR_SIZE && gstate.cursor < fields.size()) {
		...
		FlatVector::GetData<string_t>(output.data[0])[row] = StringVector::AddString(output.data[0], f.name);
		FlatVector::GetData<string_t>(output.data[1])[row] = StringVector::AddString(output.data[1], f.sf_type);
...
TableFunction GetSalesforceDescribeFunction() {
	TableFunction fn("salesforce_describe", {LogicalType::VARCHAR}, DescribeFunction, DescribeBind, DescribeInitGlobal);
