...
public:
	SalesforceTableEntry(Catalog &catalog, SchemaCatalogEntry &schema, CreateTableInfo &info, SalesforceConfig config,
	                     SalesforceTokenSet token, vector<SalesforceField> fields)
	    : TableCatalogEntry(catalog, schema, info), config_(std::move(config)), token_(std::move(token)),
...
public:
	SalesforceSchemaEntry(Catalog &catalog, CreateSchemaInfo &info, SalesforceConfig config, SalesforceTokenSet token)
	    : SchemaCatalogEntry(catalog, info), config_(std::move(config)), token_(std::move(token)) {
	}
	...
	optional_ptr<CatalogEntry> LookupEntry(CatalogTransaction transaction,
		...
		int depth = 1;
		Value dv;
		if (transaction.GetContext().TryGetCurrentSetting("sf_relationship_depth", dv) && !dv.IsNull()) {
...
private:
	...
	SalesforceTableEntry *BuildAndCacheEntry(const SalesforceDescribe &describe) {
		...
		for (auto &f : describe.fields) {
			...
			if (!f.nillable) {
				info.constraints.push_back(make_uniq<NotNullConstraint>(LogicalIndex(queryable.size())));
			}
			queryable.push_back(f);
		}
		auto entry = make_uniq<SalesforceTableEntry>(catalog, *this, info, config_, token_, std::move(queryable));
		auto *ptr = entry.get();
	...
	void ExpandParentRelationships(SalesforceSession &session, SalesforceDescribe &describe, int depth) {
	...
	vector<SalesforceField> BuildRelationshipFields(SalesforceSession &session, const SalesforceDescribe &desc,
	                                                int depth, std::unordered_set<string> &visited, int level) {
		...
		for (auto &f : desc.fields) {
			...
			if (!pd) {
				RelDiagRecord(f.relationship_name, parent, level, "skipped", "parent_not_describable", -1);
				continue;
			}
			SalesforceField rel;
	...
	void EnsureObjectListLoaded(ClientContext &context) {
		...
		for (auto &obj : session.GlobalDescribe()) {
			...
			listing_.emplace(key, make_uniq<SalesforceTableEntry>(catalog, *this, info, config_, token_,
			                                                      vector<SalesforceField> {}));
		}
...
private:
	[[noreturn]] static void Unsupported(const char *op) {
		throw NotImplementedException(std::string("Salesforce ATTACH catalog is read-only — ") + op +
		                              " is not supported.");
	}
...
public:
	...
	const SalesforceConfig &GetConfig() const {
		return config_;
	}
	const SalesforceTokenSet &GetToken() const {
		return token_;
	}
	...
	PhysicalOperator &PlanMergeInto(ClientContext &, PhysicalPlanGenerator &, LogicalMergeInto &,
	                                PhysicalOperator &) override {
...
public:
	SalesforceTransaction(TransactionManager &manager, ClientContext &context) : Transaction(manager, context) {
	}
};
...
static unique_ptr<Catalog> SalesforceAttach(optional_ptr<StorageExtensionInfo>, ClientContext &context,
                                            AttachedDatabase &db, const string &, AttachInfo &attach_info,
                                            AttachOptions &) {
	// #2 parse + validate; #3 OAuth exchange (token in memory only).
	SalesforceConfig config = SalesforceConfig::ParseAndValidate(attach_info.path, attach_info, context);
...
static unique_ptr<TransactionManager> SalesforceCreateTransactionManager(optional_ptr<StorageExtensionInfo>,
...
void GetSalesforceCatalogCredentials(ClientContext &context, const string &alias, SalesforceConfig &cfg,
	...
	if (!catalog) {
		throw BinderException("salesforce_aggregate: no attached catalog named '%s' — ATTACH a "
		                      "Salesforce org first, then pass its alias.",
		                      alias);
	}
	if (catalog->GetCatalogType() != "salesforce") {
		throw BinderException("salesforce_aggregate: catalog '%s' is not a Salesforce catalog.", alias);
	}
...
SalesforceMetadataEngine &GetSalesforceCatalogMetadataEngine(ClientContext &context, const string &alias) {
	Catalog *catalog = nullptr;
	try {
		catalog = &Catalog::GetCatalog(context, alias);
	} catch (...) {
...
void RefreshSalesforceCatalogMetadata(ClientContext &context, const string &alias, const string &object) {
	Catalog *catalog = nullptr;
	try {
		catalog = &Catalog::GetCatalog(context, alias);
	} catch (...) {
	...
	if (!catalog) {
		throw BinderException("salesforce_refresh_metadata: no attached catalog named '%s' — ATTACH "
		                      "a Salesforce org first, then pass its alias.",
		                      alias);
	}
	if (catalog->GetCatalogType() != "salesforce") {
		throw BinderException("salesforce_refresh_metadata: catalog '%s' is not a Salesforce catalog.", alias);
	}
...
string GetSalesforceObjectDescribeJson(ClientContext &context, const string &alias, const string &object) {
	...
	if (!catalog) {
		throw BinderException("salesforce metadata: no attached catalog named '%s' — ATTACH a "
		                      "Salesforce org first, then pass its alias.",
		                      alias);
	}
	if (catalog->GetCatalogType() != "salesforce") {
		throw BinderException("salesforce metadata: catalog '%s' is not a Salesforce catalog.", alias);
	}
...
unique_ptr<FunctionData> RefreshMetadataBind(ClientContext &, TableFunctionBindInput &input,
	...
	if (args.size() < 1 || args.size() > 2) {
		throw BinderException("salesforce_refresh_metadata(catalog [, object]) takes 1 or 2 arguments.");
	}
	for (idx_t i = 0; i < args.size(); i++) {
		if (args[i].IsNull()) {
			throw BinderException("salesforce_refresh_metadata: argument %llu must not be NULL.",
...
unique_ptr<GlobalTableFunctionState> RefreshMetadataInit(ClientContext &, TableFunctionInitInput &) {
	return make_uniq<RefreshMetadataGlobalState>();
}

void RefreshMetadataFunction(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
...
TableFunction GetSalesforceRefreshMetadataFunction() {
	TableFunction fn("salesforce_refresh_metadata", {LogicalType::VARCHAR}, RefreshMetadataFunction,
	                 RefreshMetadataBind, RefreshMetadataInit);
