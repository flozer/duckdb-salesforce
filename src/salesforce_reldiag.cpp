...
const char *kOverfetchNote = "parent fetched as a full STRUCT (all queryable parent fields); nested "
...
string BuildConfigNote(const RelSnapshot &s, int64_t expanded, int64_t skipped) {
	...
	return StringUtil::Format("expanded %lld, skipped %lld parent relationship(s) up to depth %lld",
...
unique_ptr<FunctionData> RelationshipsBind(ClientContext &, TableFunctionBindInput &, vector<LogicalType> &return_types,
                                           vector<string> &names) {
	names = {"row_type",
	         "object",
	         "relationships_mode",
...
TableFunction GetSalesforceRelationshipsFunction() {
	return TableFunction("salesforce_relationships", {}, RelationshipsFunction, RelationshipsBind, RelationshipsInit);
}
