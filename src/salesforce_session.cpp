...
HttpResponse SalesforceSession::AuthorizedSend(bool post, const string &path, const string &json_body) {
	auto do_req = [&]() {
		HttpRequest req;
		req.url = TrimTrailingSlash(token_.instance_url) + path;
		req.headers = {{"Authorization", "Bearer " + token_.access_token}, {"Accept", "application/json"}};
		if (post) {
	...
	if (!resp.transport_ok) {
		throw IOException("salesforce: request to %s failed to reach the server (%s).", path, resp.transport_error);
	}
	if (resp.status == 401) {
		...
		if (resp.status == 401) {
			throw IOException("salesforce: authentication failed (HTTP 401) after refreshing the token.");
		}
	}
	...
	if (resp.status >= 400 && sfjson::GetString(resp.body, "errorCode") == "REQUEST_LIMIT_EXCEEDED") {
		throw IOException("salesforce: daily API request limit exhausted (REQUEST_LIMIT_EXCEEDED). This "
...
SalesforceQuotaSnapshot SalesforceSession::QueryLimits() {
	...
	try {
		HttpResponse resp = AuthorizedSend(false, "/services/data/" + config_.api_version + "/limits", "");
		if (resp.status != 200) {
...
string SalesforceSession::DescribeJson(const string &object) {
	string path = "/services/data/" + config_.api_version + "/sobjects/" + object + "/describe";
...
bool SalesforceSession::ToolingDescribe(const vector<string> &objects,
	...
	for (size_t start = 0; start < objects.size(); start += kChunk) {
		...
		string soql = "SELECT EntityDefinition.QualifiedApiName, QualifiedApiName, DataType, "
		              ...
		              in_list + ")";
		string path = "/services/data/" + config_.api_version + "/tooling/query?q=" + UrlEncodeComponent(soql);
...
[[noreturn]] static void ThrowBulkError(const char *stage, const HttpResponse &r) {
	...
	throw IOException("salesforce bulk %s failed (HTTP %d): %s%s%s.", stage, r.status, code, msg.empty() ? "" : " - ",
	                  msg);
}
...
bool SalesforceSession::TryMinMaxId(const string &object, const string &where, string &min_id, string &max_id) {
	try {
...
string SalesforceSession::RunReport(const string &report_id) {
	...
	string path = "/services/data/" + config_.api_version + "/analytics/reports/" + report_id + "?includeDetails=true";
	return AuthorizedGet(path);
}

string SalesforceSession::DescribeReport(const string &report_id) {
	string path = "/services/data/" + config_.api_version + "/analytics/reports/" + report_id + "/describe";
	return AuthorizedGet(path);
}
...
SalesforceQueryResult SalesforceSession::Query(const string &soql) {
	...
	while (true) {
		...
		if (result.page_count >= kMaxPages) {
			throw IOException("salesforce query aborted: exceeded the maximum page count (%llu).",
			                  static_cast<unsigned long long>(kMaxPages));
		}
		if (!seen_cursors.insert(pg.next_path).second) {
			throw IOException("salesforce query aborted: pagination loop detected (nextRecordsUrl "
			                  "repeated).");
		}
		path = pg.next_path;
	}
