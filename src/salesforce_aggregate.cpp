...
bool IsAggregateFn(const string &fn_upper) {
	return fn_upper == "COUNT" || fn_upper == "COUNT_DISTINCT" || fn_upper == "SUM" || fn_upper == "AVG" ||
...
void RejectUnsafe(const string &what, const string &v) {
	...
	if (StringUtil::Contains(StringUtil::Upper(v), "SELECT")) {
		throw BinderException("salesforce_aggregate: %s must not contain a nested SELECT.", what);
	}
}
...
AggColumn ParseTerm(const string &term_raw, idx_t &unaliased) {
	...
	auto lp = term.find('(');
	if (lp == string::npos) {
		throw BinderException("salesforce_aggregate: '%s' is not an aggregate — each term must be "
		                      "MIN/MAX/SUM/AVG/COUNT/COUNT_DISTINCT(field).",
		                      term);
	}
	string fn = StringUtil::Upper(Trim(term.substr(0, lp)));
	if (!IsAggregateFn(fn)) {
		throw BinderException("salesforce_aggregate: '%s' is not a supported aggregate function "
	...
	if (alias.empty()) {
		...
		if (!IsIdentifier(alias)) {
			throw BinderException("salesforce_aggregate: alias '%s' is not a valid identifier.", alias);
		}
		col.name = alias;
		col.key = alias;
	}
...
unique_ptr<FunctionData> AggBind(ClientContext &context, TableFunctionBindInput &input,
	...
	if (args.size() < 3 || args.size() > 5) {
		throw BinderException("salesforce_aggregate(catalog, object, aggregates [, filter [, "
		                      "group_by]]) takes 3 to 5 arguments.");
	}
	for (idx_t i = 0; i < args.size(); i++) {
		if (args[i].IsNull()) {
			throw BinderException("salesforce_aggregate: argument %llu must not be NULL.", (unsigned long long)(i + 1));
		}
	}
	...
	if (!IsIdentifier(object)) {
		throw BinderException("salesforce_aggregate: object '%s' is not a valid sObject name.", object);
	}
	if (aggregates.empty()) {
	...
	if (!group_by.empty()) {
		...
		for (auto &g : SplitTopLevelCommas(group_by)) {
			...
			if (!IsIdentifier(field)) {
				throw BinderException("salesforce_aggregate: group_by '%s' must be a simple field "
...
TableFunction GetSalesforceAggregateFunction() {
	TableFunction fn("salesforce_aggregate", {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR},
