...
static vector<string> BuildIdRangeWheres(const string &pushed, const string &min_id, const string &max_id, int64_t n) {
...
static void EmitRelationshipSoql(const SalesforceField &rel, const string &path, vector<string> &out) {
...
static unique_ptr<FunctionData> ScanBind(ClientContext &, TableFunctionBindInput &, vector<LogicalType> &,
                                         vector<string> &) {
	throw InternalException("salesforce_scan is catalog-internal and cannot be called directly");
}
...
static unique_ptr<GlobalTableFunctionState> ScanInitGlobal(ClientContext &context, TableFunctionInitInput &input) {
		...
		if (aggregate_only) {
			...
			if (!probe) {
				...
				if (gstate->session->TryEstimateCount(count_soql, n)) {
					...
					reason = StringUtil::Format("auto: est %lld rows %s threshold %lld -> %s", (long long)n,
					                            (n > threshold) ? ">" : "<=", (long long)threshold, effective.c_str());
				} else {
	...
	if (!base64_field.empty()) {
		if (transport == "bulk") {
			throw BinderException("projected base64 field '%s' is not supported by Bulk API 2.0 CSV; "
			                      "use 'rest' or 'auto'.",
			                      base64_field);
		}
		if (effective == "bulk") {
			effective = "rest";
			reason = StringUtil::Format("auto: bulk-incompatible (projected base64 field '%s') -> rest", base64_field);
		}
	}
	...
	if (effective == "bulk") {
		...
		for (auto &w : wheres) {
			gstate->bulk_chunk_soqls.push_back(BuildSelectSoql(bind.object, select_fields, w, optional_idx()));
		}
	...
	while (true) {
		...
		if (!last) {
			...
			if (!g.seen.insert(pg.next_path).second) {
				throw IOException("salesforce scan: pagination loop detected (nextRecordsUrl repeated).");
			}
...
static bool BulkAdvancePage(ScanGlobalState &g, ScanLocalState &l) {
	while (true) {
		...
		string path = l.started ? (l.results_base + "?locator=" + l.next_locator) : l.results_base;
...
static void AppendBulkStruct(Vector &vec, idx_t row, const SalesforceField &field, const string &path,
		...
		if (lstate.cursor >= lstate.page.size()) {
			...
			while (!got) {
				if (!lstate.has_chunk) {
					...
					QuotaGuardBulkStart(context, *lstate.session, lstate.session->Token().instance_url);
		...
		while (row < STANDARD_VECTOR_SIZE && lstate.cursor < lstate.page.size()) {
			...
			for (idx_t j = 0; j < gstate.column_ids.size(); j++) {
				...
				if (col < bind.fields.size() && bind.fields[col].is_relationship) {
					...
					AppendBulkStruct(output.data[j], row, bind.fields[col], bind.fields[col].relationship_name, cells,
					                 lstate.columns);
					continue;
				}
...
static bool ExplainFieldOf(const Expression &expr, const vector<SalesforceField> &fields,
	...
	std::function<void(const Expression &)> walk = [&](const Expression &e) {
		...
		ExpressionIterator::EnumerateChildren(e, [&](const Expression &child) { walk(child); });
	};
	walk(expr);
...
static void ScanPushdownComplexFilter(ClientContext &, LogicalGet &get, FunctionData *bind_data,
	...
	if (!where_part.empty()) {
		bind.pushed_where = bind.pushed_where.empty() ? where_part : bind.pushed_where + " AND " + where_part;
	}
