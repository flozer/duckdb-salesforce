...
static bool FlipComparison(ExpressionType in, ExpressionType &out) {
	...
	case ExpressionType::COMPARE_NOTEQUAL:
		...
		return true;
	case ExpressionType::COMPARE_LESSTHAN:
		out = ExpressionType::COMPARE_GREATERTHAN;
		return true;
	case ExpressionType::COMPARE_GREATERTHAN:
		out = ExpressionType::COMPARE_LESSTHAN;
		return true;
	case ExpressionType::COMPARE_LESSTHANOREQUALTO:
		out = ExpressionType::COMPARE_GREATERTHANOREQUALTO;
		return true;
	case ExpressionType::COMPARE_GREATERTHANOREQUALTO:
		out = ExpressionType::COMPARE_LESSTHANOREQUALTO;
		return true;
	default:
		return false;
	}
...
static bool TranslateExpr(const Expression &expr, const vector<SalesforceField> &fields,
	...
	try {
		...
		if (klass == ExpressionClass::BOUND_COMPARISON) {
			...
			out = field + " " + op + " " + SoqlLiteral(constant->Cast<BoundConstantExpression>().value);
			return true;
		}
		...
		if (klass == ExpressionClass::BOUND_BETWEEN) {
			...
			string lo_op, hi_op;
			if (!ComparisonOp(bw.LowerComparisonType(), lo_op) || !ComparisonOp(bw.UpperComparisonType(), hi_op)) {
				return false;
			}
			string lo = SoqlLiteral(lo_val);
			string hi = SoqlLiteral(hi_val);
			out = "(" + field + " " + lo_op + " " + lo + " AND " + field + " " + hi_op + " " + hi + ")";
			return true;
		}

		// IS NULL / IS NOT NULL (exact).
		if (klass == ExpressionClass::BOUND_OPERATOR &&
		    (etype == ExpressionType::OPERATOR_IS_NULL || etype == ExpressionType::OPERATOR_IS_NOT_NULL)) {
			...
			out = field + (etype == ExpressionType::OPERATOR_IS_NULL ? " = null" : " != null");
			return true;
		}
		...
		if (klass == ExpressionClass::BOUND_FUNCTION) {
			...
			bool like = (fn == "~~"), pre = (fn == "prefix"), suf = (fn == "suffix"), con = (fn == "contains");
		...
		if (klass == ExpressionClass::BOUND_CONJUNCTION &&
		    (etype == ExpressionType::CONJUNCTION_OR || etype == ExpressionType::CONJUNCTION_AND)) {
...
void PushdownToSoql(const vector<SalesforceField> &fields, const vector<idx_t> &projection_to_field, string &out_where,
                    vector<unique_ptr<Expression>> &filters, vector<PushdownConjunctInfo> *out_info) {
	out_where.clear();
...
static unique_ptr<GlobalTableFunctionState> LastSoqlInit(ClientContext &, TableFunctionInitInput &) {
...
static void LastSoqlFunction(ClientContext &, TableFunctionInput &data, DataChunk &output) {
	...
	FlatVector::GetData<string_t>(output.data[0])[0] = StringVector::AddString(output.data[0], soql);
	gstate.emitted = true;
...
static unique_ptr<GlobalTableFunctionState> LastPagesInit(ClientContext &, TableFunctionInitInput &) {
...
TableFunction GetSalesforceLastScanPagesFunction() {
	return TableFunction("salesforce_last_scan_pages", {}, LastPagesFunction, LastPagesBind, LastPagesInit);
}
...
static unique_ptr<GlobalTableFunctionState> LastBulkBodyInit(ClientContext &, TableFunctionInitInput &) {
...
static void LastBulkBodyFunction(ClientContext &, TableFunctionInput &data, DataChunk &output) {
	...
	FlatVector::GetData<string_t>(output.data[0])[0] = StringVector::AddString(output.data[0], body);
	gstate.emitted = true;
	output.SetCardinality(1);
}
...
TableFunction GetSalesforceLastBulkCreateBodyFunction() {
	return TableFunction("salesforce_last_bulk_create_body", {}, LastBulkBodyFunction, LastBulkBodyBind,
	                     LastBulkBodyInit);
}
...
static unique_ptr<GlobalTableFunctionState> LastTransportInit(ClientContext &, TableFunctionInitInput &) {
...
static void LastTransportFunction(ClientContext &, TableFunctionInput &data, DataChunk &output) {
	...
	FlatVector::GetData<string_t>(output.data[0])[0] = StringVector::AddString(output.data[0], transport);
	if (est < 0) {
		FlatVector::SetNull(output.data[1], 0, true);
	} else {
		FlatVector::GetData<int64_t>(output.data[1])[0] = est;
	}
	FlatVector::GetData<string_t>(output.data[2])[0] = StringVector::AddString(output.data[2], reason);
	gstate.emitted = true;
	output.SetCardinality(1);
}
...
TableFunction GetSalesforceLastTransportFunction() {
	return TableFunction("salesforce_last_transport", {}, LastTransportFunction, LastTransportBind, LastTransportInit);
}
...
static unique_ptr<GlobalTableFunctionState> DescribeCallsInit(ClientContext &, TableFunctionInitInput &) {
...
TableFunction GetSalesforceDescribeCallsFunction() {
	return TableFunction("salesforce_describe_calls", {}, DescribeCallsFunction, DescribeCallsBind, DescribeCallsInit);
}
...
static unique_ptr<GlobalTableFunctionState> ToolingCallsInit(ClientContext &, TableFunctionInitInput &) {
...
TableFunction GetSalesforceToolingCallsFunction() {
	return TableFunction("salesforce_tooling_calls", {}, ToolingCallsFunction, ToolingCallsBind, ToolingCallsInit);
}
...
static unique_ptr<GlobalTableFunctionState> GlobalCallsInit(ClientContext &, TableFunctionInitInput &) {
...
TableFunction GetSalesforceGlobalDescribeCallsFunction() {
	return TableFunction("salesforce_global_describe_calls", {}, GlobalCallsFunction, GlobalCallsBind, GlobalCallsInit);
}
