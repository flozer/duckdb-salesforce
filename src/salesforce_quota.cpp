...
unique_ptr<FunctionData> LastQuotaBind(ClientContext &, TableFunctionBindInput &, vector<LogicalType> &return_types,
                                       vector<string> &names) {
...
void LastQuotaFunction(ClientContext &, TableFunctionInput &data, DataChunk &output) {
	...
	FlatVector::GetData<string_t>(output.data[5])[0] = StringVector::AddString(output.data[5], reason);
	gstate.emitted = true;
...
void QuotaGuardBulkStart(ClientContext &context, SalesforceSession &session, const string &instance_url) {
	...
	if (!snap.available) {
		if (fail_open) {
			RecordDecision("", -1, -1, -1, true, string("limits unavailable -> allowed (fail-open)") + src);
			DiagSetQuota(-1, true);
			return;
		}
		RecordDecision("", -1, -1, -1, false, string("limits unavailable -> blocked (fail-closed)") + src);
		DiagSetQuota(-1, false);
		throw IOException("salesforce quota guard: /limits is unavailable and sf_quota_fail_open=false — "
	...
	if (ok) {
		RecordDecision("DailyApiRequests", snap.api_max, snap.api_remaining, threshold, true,
		               StringUtil::Format("allowed: Remaining %lld > threshold %lld%s", (long long)snap.api_remaining,
		                                  (long long)threshold, src));
		DiagSetQuota(snap.api_remaining, true);
		return;
	}

	string reason =
	    StringUtil::Format("DailyApiRequests Remaining %lld <= threshold %lld (reserve %lld%% of %lld, min %lld)%s%s",
	...
	throw IOException("salesforce quota guard: %s — refusing to start a Bulk job. SET sf_quota_enforce=false "
...
TableFunction GetSalesforceLastQuotaFunction() {
	return TableFunction("salesforce_last_quota", {}, LastQuotaFunction, LastQuotaBind, LastQuotaInit);
}
