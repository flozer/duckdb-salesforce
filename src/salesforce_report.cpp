...
static const char *kReportsSoql = "SELECT Id, Name, DeveloperName, FolderName, Format FROM Report";
...
static unique_ptr<FunctionData> ReportsBind(ClientContext &context, TableFunctionBindInput &input,
                                            vector<LogicalType> &return_types, vector<string> &names) {
	if (input.inputs.empty()) {
		throw BinderException("salesforce_reports(catalog) requires the attached Salesforce catalog "
...
static unique_ptr<GlobalTableFunctionState> ReportsInit(ClientContext &, TableFunctionInitInput &) {
	return make_uniq<ReportsGlobalState>();
}

static void ReportsFunction(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
	...
	while (gs.cursor < gs.records.size() && produced < STANDARD_VECTOR_SIZE) {
		...
		for (idx_t c = 0; c < 5; c++) {
			...
			if (!found || is_null) {
				...
				FlatVector::GetData<string_t>(output.data[c])[produced] = StringVector::AddString(output.data[c], val);
			}
		}
		gs.cursor++;
		produced++;
...
static const char *kReportGuidance = "report result is a validation sample only (max 2000 rows, no pagination); "
...
static unique_ptr<FunctionData> ReportBind(ClientContext &context, TableFunctionBindInput &input,
	...
	if (args.size() < 2) {
		throw BinderException("salesforce_report(catalog, report_id) requires the attached catalog "
	...
	if (tt.empty()) {
		throw BinderException("salesforce_report: only tabular reports are supported in this cut "
...
static unique_ptr<GlobalTableFunctionState> ReportInit(ClientContext &, TableFunctionInitInput &) {
...
static bool IsAmbiguousLiteral(const string &v) {
	...
	if (v.size() >= 8 && v[4] == '-' && v[0] >= '0' && v[0] <= '9' && v[1] >= '0' && v[1] <= '9' && v[2] >= '0' &&
	    v[2] <= '9' && v[3] >= '0' && v[3] <= '9') {
		return true;
	}
	return false;
...
// Token -> field-API-name resolution (BuiltinReportToken / NormalizeSnakeToken /
...
static string DominantColumnPrefix(const vector<string> &columns, const vector<ReportFilter> &filters) {
...
static string ResolveReportTokenCandidate(ClientContext &context, const string &base, const string &token) {
...
static unique_ptr<FunctionData> ReportSoqlBind(ClientContext &context, TableFunctionBindInput &input,
                                               vector<LogicalType> &return_types, vector<string> &names) {
	auto &args = input.inputs;
	if (args.size() < 2) {
		throw BinderException("salesforce_report_soql(catalog, report_id) requires the attached "
	...
	if (bind->translatable) {
		if (candidates.empty()) {
			...
			try {
				...
				for (auto &c : candidates) {
					if (IsSafeIdentifier(c.obj) && engine.IsQueryable(context, c.obj)) {
						base = c.obj;
						by = c.by;
				...
				if (base.empty()) {
					...
					caveats.push_back("base object could not be resolved to a queryable "
					                  "object in Describe Global (report type '" +
					                  rt + "')");
				} else {
					bind->base_object = base;
					string by_s = by;
					bind->base_object_resolved_by = (by_s == "column_prefix") ? "column_prefix_hint" : by_s;
					string label = (by_s == "custom_entity_suffix") ? "CustomEntity suffix (custom_entity_suffix)"
					               : (by_s == "builtin_report_type_map")
					                   ? "builtin report type map (builtin_report_type_map)"
					                   : "column prefix (column_prefix, low confidence)";
					caveats.push_back("base object resolved from " + label + ": '" + rt + "' -> '" + base + "'");
	...
	// SINGLE-HOP relationship field, via the shared metadata engine. out = the
	// SOQL field (bare or "Rel.Field"); out_filterable = filterability on its
	// (related) object; out_rel flags a relationship attempt; out_related names
	// the related object for diagnostics. False -> caller sets translatable=false.
	auto resolve_field = [&](const string &token, string &out, bool &out_filterable, bool &out_rel,
	                         string &out_related) -> bool {
		out_rel = false;
		out_related.clear();
		auto dot = token.find('.');
		if (dot == string::npos) {
			...
			if (!candidate.empty() && engine.ResolveField(context, base, candidate, out, out_filterable)) {
				return true;
			}
			return false;
		}
		...
		if (StringUtil::CIEquals(prefix, base)) {
			...
			return engine.ResolveField(context, base, rest, out, out_filterable);
		}
		...
		string target, real_rel;
		if (!engine.ResolveRelationship(context, base, prefix, target, real_rel)) {
			return false; // unknown / polymorphic / non-queryable relationship
		}
		string real;
		if (!engine.ResolveField(context, target, rest, real, out_filterable)) {
			return false; // final field absent on the related object
		}
		out = real_rel + "." + real; // realRelationshipName.realField
		out_related = target;
		return true;
	};
	...
	if (object_validated) {
		for (auto &c : bind->columns) {
			...
			if (!resolve_field(c, field, filt, was_rel, rel_obj)) {
				...
				caveats.push_back("column '" + c + "' does not resolve to a field on '" + base + "'");
				continue;
			}
	...
	for (auto &f : bind->filters) {
		...
		if (!resolve_field(f.field, field, filt, was_rel, rel_obj)) {
			...
			caveats.push_back("filter field '" + f.field + "' does not resolve to a field on '" + base + "'");
			continue;
		}
		...
		if (!filt) {
			...
			if (!rel_obj.empty()) {
				caveats.push_back("filter field '" + f.field + "' is not filterable on related object '" + rel_obj +
				                  "'");
			} else {
				caveats.push_back("filter field '" + f.field + "' is not filterable on '" + base + "'");
			}
			continue;
		}
		if (is_like) {
...
static unique_ptr<GlobalTableFunctionState> ReportSoqlInit(ClientContext &, TableFunctionInitInput &) {
...
static void ReportSoqlFunction(ClientContext &, TableFunctionInput &data, DataChunk &output) {
	...
	auto str = [&](idx_t col, const string &v) {
		FlatVector::GetData<string_t>(output.data[col])[0] = StringVector::AddString(output.data[col], v);
	};
	str(0, bd.report_id);
...
TableFunction GetSalesforceReportsFunction() {
	return TableFunction("salesforce_reports", {LogicalType::VARCHAR}, ReportsFunction, ReportsBind, ReportsInit);
}

TableFunction GetSalesforceReportFunction() {
	return TableFunction("salesforce_report", {LogicalType::VARCHAR, LogicalType::VARCHAR}, ReportFunction, ReportBind,
	                     ReportInit);
}

TableFunction GetSalesforceReportSoqlFunction() {
	return TableFunction("salesforce_report_soql", {LogicalType::VARCHAR, LogicalType::VARCHAR}, ReportSoqlFunction,
