...
static string RequireEnv(ClientContext &context, const char *name) {
	...
	if (!EnvLookup(context, name, v)) {
		throw BinderException("salesforce ATTACH: missing required environment variable '%s'.", name);
	}
	return v;
}
...
static string RequireOption(const case_insensitive_map_t<string> &opts, const char *key) {
	auto it = opts.find(key);
	if (it == opts.end() || Trimmed(it->second).empty()) {
		throw BinderException("salesforce ATTACH: missing required option '%s'. Expected: "
...
static void ValidateLoginUrl(const string &url) {
	...
	if (!StringUtil::StartsWith(StringUtil::Lower(url), kHttps) || url.size() <= kHttps.size()) {
		throw BinderException("salesforce ATTACH: 'login_url' must be an absolute https:// URL "
		                      "(got: '%s').",
		                      url);
	}
}
...
static string NormaliseApiVersion(const string &raw) {
	...
	if (!seen_digit || !seen_dot || !digit_after_dot) {
		throw BinderException("salesforce ATTACH: 'api_version' must look like 'vNN.N' or 'NN.N' "
		                      "(got: '%s').",
		                      raw);
	}
	return "v" + v.substr(i);
...
static void ParseSfdxAuthUrl(const string &url, SalesforceConfig &cfg) {
	...
	auto fail = []() {
		throw BinderException("salesforce ATTACH: SF_SFDX_AUTH_URL is not a valid SFDX auth URL "
...
static string LoadPrivateKeyFile(const string &pathv) {
	...
	if (!f) {
		throw BinderException("salesforce ATTACH (jwt): cannot open private key file '%s'.", pathv);
	}
	std::stringstream ss;
	ss << f.rdbuf();
	string pem = ss.str();
	if (pem.find("-----BEGIN") == string::npos) {
		throw BinderException("salesforce ATTACH (jwt): file '%s' is not a PEM private key.", pathv);
	}
	if (pem.find("ENCRYPTED") != string::npos) {
		throw BinderException("salesforce ATTACH (jwt): encrypted private keys are not supported; "
...
SalesforceConfig SalesforceConfig::ParseAndValidate(const string &path, AttachInfo &info, ClientContext &context) {
	SalesforceConfig cfg;

	// --- path: salesforce://<org> ------------------------------------------
	if (!StringUtil::StartsWith(StringUtil::Lower(path), kScheme)) {
		throw BinderException("salesforce ATTACH: path must start with 'salesforce://' "
		                      "(got: '%s').",
		                      path);
	}
	cfg.org = Trimmed(path.substr(string(kScheme).size()));
	if (cfg.org.empty()) {
		throw BinderException("salesforce ATTACH: missing org in path. Expected "
	...
	if (auth_source == "options") {
		...
		if (pk_it != opts.end() && !Trimmed(pk_it->second).empty()) {
			...
			throw BinderException("salesforce ATTACH (jwt): set 'private_key_file' or the "
		...
		throw BinderException("salesforce ATTACH: 'auth_source' must be 'options', 'env', "
