...
private:
	...
	HttpResponse Send(Method method, const HttpRequest &request) {
		...
		for (int attempt = 1; attempt <= kMaxAttempts; attempt++) {
			...
			httplib::Result res = (method == Method::POST) ? cli.Post(url.path, headers, request.body, content_type)
			...
			if (err == httplib::Error::SSLServerVerification) {
				...
				return TransportError("TLS certificate verification failed. On macOS, OpenSSL has "
...
public:
	...
	ScriptedMockHttpClient(int token_status, string token_body, vector<int> describe_statuses,
	                       vector<string> describe_bodies, vector<int> query_statuses, vector<string> query_bodies,
	                       vector<int> global_statuses, vector<string> global_bodies, vector<int> count_statuses,
	                       vector<string> count_bodies, vector<int> limits_statuses, vector<string> limits_bodies,
	                       vector<int> tooling_statuses, vector<string> tooling_bodies, vector<int> queryall_statuses,
	                       vector<string> queryall_bodies, BulkMock bulk, ReportMock report)
	    : token_status_(token_status), token_body_(std::move(token_body)),
	      describe_statuses_(std::move(describe_statuses)), describe_bodies_(std::move(describe_bodies)),
	      query_statuses_(std::move(query_statuses)), query_bodies_(std::move(query_bodies)),
	      global_statuses_(std::move(global_statuses)), global_bodies_(std::move(global_bodies)),
	      count_statuses_(std::move(count_statuses)), count_bodies_(std::move(count_bodies)),
	      limits_statuses_(std::move(limits_statuses)), limits_bodies_(std::move(limits_bodies)),
	      tooling_statuses_(std::move(tooling_statuses)), tooling_bodies_(std::move(tooling_bodies)),
	      queryall_statuses_(std::move(queryall_statuses)), queryall_bodies_(std::move(queryall_bodies)),
	      bulk_(std::move(bulk)), report_(std::move(report)) {
	}
	...
	HttpResponse Get(const HttpRequest &request) override {
		...
		if (request.url.find("/analytics/reports/") != string::npos) {
			if (request.url.find("/describe") != string::npos) {
				return Step(report_.describe_statuses, report_.describe_bodies, report_describe_index_);
			}
		...
		if (request.url.find("/queryAll") != string::npos) {
			return Step(queryall_statuses_, queryall_bodies_, queryall_index_);
		}
		return Step(query_statuses_, query_bodies_, query_index_);
	}

private:
	static HttpResponse Step(const vector<int> &statuses, const vector<string> &bodies, size_t &index) {
		HttpResponse r;
		r.transport_ok = true;
...
unique_ptr<SalesforceHttpClient> BuildHttpClientForContext(ClientContext &context) {
	...
	if (token_status != 0) {
		...
		vector<int> qa_status = ParseIntCsv(SettingStr(context, "sf_mock_queryall_status"));
		if (qa_status.empty()) {
			qa_status.push_back(200);
		}
		vector<string> qa_body = SplitOn(SettingStr(context, "sf_mock_queryall_body"), "|~|");
		...
		report.describe_statuses = ParseIntCsv(SettingStr(context, "sf_mock_report_describe_status"));
		if (report.describe_statuses.empty()) {
			report.describe_statuses.push_back(200);
		}
		report.describe_bodies = SplitOn(SettingStr(context, "sf_mock_report_describe_body"), "|~|");

		return make_uniq_base<SalesforceHttpClient, ScriptedMockHttpClient>(
		    static_cast<int>(token_status), SettingStr(context, "sf_mock_token_body"), std::move(d_status),
		    std::move(d_body), std::move(q_status), std::move(q_body), std::move(g_status), std::move(g_body),
