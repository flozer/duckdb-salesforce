...
static SalesforceTokenSet HandleTokenResponse(const HttpResponse &resp, const string &url) {
	if (!resp.transport_ok) {
		throw IOException("salesforce OAuth: token request failed to reach %s (%s).", url, resp.transport_error);
	}
	if (resp.status != 200) {
		...
		throw IOException("salesforce OAuth token exchange failed (HTTP %d): %s%s%s%s.", resp.status, err,
		                  desc.empty() ? "" : " - ", desc, hint);
	}
	...
	if (ts.access_token.empty()) {
		throw IOException("salesforce OAuth: token response did not contain an access_token.");
	}
	if (ts.instance_url.empty()) {
		throw IOException("salesforce OAuth: token response did not contain an instance_url.");
	}
	return ts;
}

SalesforceTokenSet SalesforceAuth::ExchangeRefreshToken(const SalesforceConfig &config, SalesforceHttpClient &client) {
	HttpRequest req;
	...
	req.body = "grant_type=refresh_token" + string("&client_id=") + FormEncode(config.client_id) +
	           "&client_secret=" + FormEncode(config.client_secret) +
	           "&refresh_token=" + FormEncode(config.refresh_token);
...
static string SignRs256(const string &pem, const string &input) {
	...
	BIO_free(bio);
	if (!pkey) {
		throw IOException("salesforce OAuth (jwt): could not parse the private key (expected an "
	...
	size_t siglen = 0;
	if (ok) {
		ok = EVP_DigestSign(ctx, nullptr, &siglen, reinterpret_cast<const unsigned char *>(input.data()),
		                    input.size()) == 1;
	}
	if (ok) {
		sig.resize(siglen);
	...
	EVP_PKEY_free(pkey);
	if (!ok) {
		throw IOException("salesforce OAuth (jwt): failed to RS256-sign the assertion (check "
...
SalesforceTokenSet SalesforceAuth::ExchangeJwtBearer(const SalesforceConfig &config, SalesforceHttpClient &client) {
	// Short-lived assertion: exp = now + 300s (Salesforce caps the window).
	int64_t now = static_cast<int64_t>(std::time(nullptr));
	int64_t exp = now + 300;

	string header = "{\"alg\":\"RS256\",\"typ\":\"JWT\"}";
	string claims = "{\"iss\":\"" + JsonEscape(config.client_id) + "\",\"sub\":\"" + JsonEscape(config.username) +
	                "\",\"aud\":\"" + JsonEscape(TrimTrailingSlash(config.login_url)) +
	                "\",\"iat\":" + std::to_string(now) + ",\"exp\":" + std::to_string(exp) + "}";
	...
	req.body = "grant_type=" + FormEncode("urn:ietf:params:oauth:grant-type:jwt-bearer") +
...
SalesforceTokenSet SalesforceAuth::AcquireToken(const SalesforceConfig &config, SalesforceHttpClient &client) {
